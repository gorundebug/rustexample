...
NAMESPACE="${KUBERNETES_NAMESPACE:-rustexample}"
...
deploy_services() {
  ...
  set -- "$@" \
    ...
    --set-string env.OTEL_EXPORTER_OTLP_ENDPOINT=http://otel-collector:4318 \
  ...
  helm upgrade --install analyticsservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/rustexample/analyticsservice" \
  ...
  helm upgrade --install automationservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/rustexample/automationservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install inventoryservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/rustexample/inventoryservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install orderservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/rustexample/orderservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
