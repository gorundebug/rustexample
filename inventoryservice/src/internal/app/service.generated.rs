...
#[macro_use]
#[path = "pipeline_inventory_item.generated.rs"]
mod pipeline_inventory_item;
use pipeline_inventory_item::{
...
pub struct ServiceStreams {
    pub inventory_item: InventoryItemPipelineStreams,
}
...
pub struct ServiceMakers {
    pub inventory_item: InventoryItemPipelineMakers,
...
impl Default for ServiceMakers {
    fn default() -> Self {
        Self {
            inventory_item: InventoryItemPipelineMakers::default(),
...
pub struct ServiceFunctions {
    pub inventory_item: InventoryItemPipelineFunctions,
}
...
pub async fn init_functions(
    ...
    let (maker_error_sender, maker_error_receiver) = mpsc::channel::<RuntimeError>();
        let get_inventory_item_data_maker = makers.inventory_item.get_inventory_item_data.clone();
        ...
        let get_inventory_item_error_maker = makers.inventory_item.get_inventory_item_error.clone();
        ...
        let get_inventory_item_error_future = async move {
            let result = (get_inventory_item_error_maker)(
                    ...
                    &config.streams.map_inventory_item_error,
                ).await;
            match result {
                Ok(value) => Some(value),
                Err(error) => {
                    get_inventory_item_error_group_context.cancel();
                    get_inventory_item_error_error_sender
                        .send(error)
                        .expect("function maker error receiver was dropped");
                    None
                }
        ...
        let process_order_item_source_maker = makers.inventory_item.process_order_item_source.clone();
    ...
    let (
        ...
        get_inventory_item_error_future,
    ...
    let get_inventory_item_error = get_inventory_item_error.ok_or_else(|| RuntimeError::InvalidConfiguration(
        "function maker get_inventory_item_error failed without an error".to_string(),
    ))?;
    ...
    Ok(ServiceFunctions {
        inventory_item: InventoryItemPipelineFunctions {
            get_inventory_item_data,
            get_inventory_item_error,
            process_order_item_source,
        },
...
pub fn init_runtime(
    ...
    let InventoryItemPipelineStreams {
        process_order_item: process_inventory_item,
        get_inventory_item_data,
        get_inventory_item_error,
        map_inventory_item_error,
        merge_inventory_result,
    } = init_inventory_item_pipeline_streams!(config, environment, functions)?;
    Ok(ServiceRuntime {
      streams: ServiceStreams {
        inventory_item: InventoryItemPipelineStreams {
      ...
      handlers: ServiceHandlers {
        process_order_item_source: functions.inventory_item.process_order_item_source,
      },
...
impl GeneratedService {
    pub async fn new(
        ...
        let process_order_item_endpoint = make_grpc_source_endpoint_consumer(
            runtime.streams.inventory_item.process_order_item.as_ref().clone(),
