...
#[derive(Clone, Default)] pub struct AnalyticsShipmentsSource;
#[async_trait]
impl DataProducer<AnalyticsEvent> for AnalyticsShipmentsSource {
    async fn start(&self, context: MessageContext, consumer: Arc<dyn Consumer<AnalyticsEvent>>) -> HandlerResult { for value in [AnalyticsEvent { key: "high-value".into(), value: 30, kind: "shipment".into() }, AnalyticsEvent { key: "standard".into(), value: 3, kind: "shipment".into() }] { consumer.consume(context.clone(), Payload::new(value)).await; } Ok(()) }
...
pub async fn make_analytics_shipments_source(_context: MessageContext, _environment: RuntimeEnvironment, _config: &CustomEndpointConfig) -> RuntimeResult<AnalyticsShipmentsSource> { Ok(AnalyticsShipmentsSource) }
