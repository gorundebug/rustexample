...
#[derive(Clone, Default)] pub struct AnalyticsOrdersSource;
#[async_trait]
impl DataProducer<AnalyticsEvent> for AnalyticsOrdersSource {
    async fn start(&self, context: MessageContext, consumer: Arc<dyn Consumer<AnalyticsEvent>>) -> HandlerResult {
        for value in [AnalyticsEvent { key: "high-value".into(), value: 10, kind: "order".into() }, AnalyticsEvent { key: "standard".into(), value: 1, kind: "order".into() }] { consumer.consume(context.clone(), Payload::new(value)).await; }
        Ok(())
    }
    async fn stop(&self, _context: MessageContext) {}
}
#[async_trait]
impl EndpointHandler<(), AnalyticsEvent, (), String> for AnalyticsOrdersSource {
...
pub async fn make_analytics_orders_source(_context: MessageContext, _environment: RuntimeEnvironment, _config: &CustomEndpointConfig) -> RuntimeResult<AnalyticsOrdersSource> { Ok(AnalyticsOrdersSource) }
