    ...
    > for InventorySink
{
    ...
    ) -> HandlerResult<(MessageContext, InventorySinkState)> {
        Ok((context, InventorySinkState::default()))
    }
    ...
    async fn consume_message(
        ...
        context: MessageContext,
        _stream: StreamContext<OrderItem, OrderItemResult, OrderState>,
        state: Arc<Mutex<InventorySinkState>>,
...
pub fn make_inventory_sink(
    _context: MessageContext,
    _environment: RuntimeEnvironment,
    _config: &GrpcEndpointConfig,
) -> RuntimeResult<InventorySink> {
    Ok(InventorySink)
}
