...
#[macro_use]
#[path = "pipeline_order.generated.rs"]
mod pipeline_order;
use pipeline_order::{
...
pub struct ServiceStreams {
    pub order: OrderPipelineStreams,
}
...
impl Default for ServiceMakers {
    fn default() -> Self {
        Self {
            order: OrderPipelineMakers::default(),
...
pub struct ServiceFunctions {
    pub order: OrderPipelineFunctions,
}
...
pub async fn init_functions(
    ...
    let (maker_error_sender, maker_error_receiver) = mpsc::channel::<RuntimeError>();
        let map_order_item_result_to_order_state_maker = makers.order.map_order_item_result_to_order_state.clone();
        ...
        let map_to_order_processed_maker = makers.order.map_to_order_processed.clone();
        ...
        let map_to_order_state_maker = makers.order.map_to_order_state.clone();
        ...
        let order_processed_endpoint_sink_maker = makers.order.order_processed_endpoint_sink.clone();
        ...
        let process_order_item_sink_maker = makers.order.process_order_item_sink.clone();
        ...
        let process_order_items_maker = makers.order.process_order_items.clone();
        ...
        let process_order_source_maker = makers.order.process_order_source.clone();
        ...
        let soft_deadline_maker = makers.order.soft_deadline.clone();
...
pub async fn init_runtime(
    ...
    } = init_order_pipeline_streams!(config, environment, functions)?;
    ...
    Ok(ServiceRuntime {
      ...
      handlers: ServiceHandlers {
        process_order_source: functions.order.process_order_source,
      },
...
impl GeneratedService {
    pub async fn new(
        ...
        runtime.data_connectors.order_service_api_data_source.add_endpoint(
            runtime.streams.order.process_order.as_ref().clone(),
